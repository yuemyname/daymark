...
> 시각을 시드로, 매분 한 장씩 다른 그림이 나오는 웹페이지.
> *daymark*는 원래 항해용 주간 표지물을 뜻한다. 이름은 마음에 안 들면 바꿔도 된다.

> **v1.1 변경**: 주기를 하루 → **1분**으로 변경했다. 시드 키는 날짜가 아니라
> 분 단위 판 키 `"2026-07-30 21:37"` (KST)다. 이 문서에서 "날짜 키"로 읽히는
> 부분은 판 키로 대체해 읽는다. 아카이브(§9.3)의 달력 구조는 하루 1,440장
> 체제에 맞게 W5에서 재설계가 필요하다.

---

## 1. 한 줄 정의

`2026-07-30 21:37`이라는 문자열 하나로부터 완전히 결정되는 그림 한 장을, 매분(KST 기준) 바꿔서 보여주는 정적 사이트.
...
// 회피 집합은 직전 2분의 "원추첨"(회피 없는 첫 추첨)으로 계산한다 —
// 최종 선택으로 만들면 과거 전체 재귀가 필요해서 O(1)로 타협.
// 드물게 3~4연속이 샐 수 있지만 어떤 키든 결과는 순수하게 결정적이다.
```

이게 없으면 같은 시스템이 계속 연속으로 나와서 "매분 새로 나온다"는 느낌이 죽는다.
...
4. 각 잉크: 방식이 정한 h, C는 0.03~0.07 (파스텔/차분), L은 mode에 따라 배경과 최소 0.35 이상 벌린다. 잉크 L 범위는 밝은 쪽으로 치우친다
...
**최근 판부터 과거로 거슬러 올라가는 그리드.** (v1.1: 하루 1,440장이 되면서 달력 그리드를 대체)
지금 이 분부터 역순으로 썸네일을 깔고, 스크롤 바닥에 닿으면 60장씩 더 불러온다.
날짜가 바뀌는 지점마다 날짜 구분선을 넣어 "언제"인지 잃지 않게 한다.

- 미래의 분은 없다 (지금이 최신)
- 썸네일은 **미리 저장하지 않고 그 자리에서 렌더**한다 (§1)
- 성능: `IntersectionObserver`로 화면에 들어온 칸만 렌더. 썸네일은 128px 고정. 한 번 렌더한 칸은 `ImageBitmap`으로 캐시 (상한 600장, 오래된 것부터 해제)
- 렌더 큐 — 프레임당 1칸. 동기로 다 돌리면 메인 스레드가 1초 이상 막힌다
- 라우팅은 해시(`#/archive`, `#/d/…`) — GitHub Pages 하위 경로에서도 서버 설정 없이 동작

### 9.4 `#/d/2026-07-30T21:37` — 판 상세
